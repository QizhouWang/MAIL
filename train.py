...
parser.add_argument('--method', default='mart',
                    help='mart mail')
parser.add_argument('--save-freq', '-s', default=1, type=int, metavar='N',
                    help='save frequency')

parser.add_argument('--bias', default=-1.5,
                    help='weighting bias term')
parser.add_argument('--slope', default=1.0,
                    help='weighting bias term')
...
def train(args, model, device, train_loader, optimizer, epoch, bias = None, slope = None):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if 'mail' not in args.method:
            ...
            if bias is None or slope is None: 
                bias, slope = args.bias, args.slope
...
def adjust_learning_rate(optimizer, epoch, model, method):
    ...
    if args.epochs == 120: 
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        if epoch == 70: torch.save(model.state_dict(), 'models/resnet75_90' + method + '.pth')
    else: 
        lr = args.lr
        if epoch >= 90:
            lr = args.lr * 0.001
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        if epoch == 55: torch.save(model.state_dict(), 'models/resnet30_60' + method + '.pth')
...
def main(bias = None, slope = None):
    model = ResNet18().to(device)
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)   
    
    begin_epoch = 0
    if 'mail' in args.method:
        if 'mart' in args.method:
            model.load_state_dict(torch.load('models/resnet75_90mart.pth' if args.epochs == 120 else 'models/resnet30_60mart.pth'))
        elif 'at' in args.method: 
            model.load_state_dict(torch.load('models/resnet75_90at.pth' if args.epochs == 120 else 'models/resnet30_60at.pth'))
        elif 'trades' in args.method: 
            model.load_state_dict(torch.load('models/resnet75_90trades.pth' if args.epochs == 120 else 'models/resnet30_60trades.pth'))
        else: 
            raise RuntimeError('invalid method name')
        begin_epoch = 75 if args.epochs == 120 else 60
        
    for epoch in range(begin_epoch, args.epochs):
        adjust_learning_rate(optimizer, epoch, model, args.method)

        start_time = time.time()
        train(args, model, device, train_loader, optimizer, epoch, bias, slope)
